...
import org.eclipse.leshan.server.californium.endpoint.coap.CoapEndpointFactory;
import org.eclipse.leshan.server.californium.endpoint.coap.CoapProtocolProvider;
import org.eclipse.leshan.server.californium.endpoint.coaps.CoapsEndpointFactory;
...
import org.eclipse.leshan.server.registration.InMemoryRegistrationStore2;
 ...
 */
public class LeshanServerBuilder2 {

    private static final Logger LOG = LoggerFactory.getLogger(LeshanServerBuilder2.class);
     ...
     */
    public LeshanServerBuilder2 setLocalAddress(String hostname, int port) {
        if (hostname == null) {
     ...
     * By default a wildcard address and the default CoAP port(5683) is used.
     */
    public LeshanServerBuilder2 setLocalAddress(InetSocketAddress localAddress) {
     ...
     *        ephemeral port in a bind operation.
     */
    public LeshanServerBuilder2 setLocalSecureAddress(String hostname, int port) {
        if (hostname == null) {
     ...
     */
    public LeshanServerBuilder2 setLocalSecureAddress(InetSocketAddress localSecureAddress) {
     ...
     * 
     */
    public LeshanServerBuilder2 setRegistrationStore(CaliforniumRegistrationStore registrationStore) {
     ...
     * 
     */
    public LeshanServerBuilder2 setSecurityStore(SecurityStore securityStore) {
     ...
     * connection.
     */
    public LeshanServerBuilder2 setAuthorizer(Authorizer authorizer) {
     ...
     */
    public LeshanServerBuilder2 setObjectModelProvider(LwM2mModelProvider objectModelProvider) {
     ...
     * {@link LeshanServerBuilder2#setCertificateChain(X509Certificate[])} should be used.
     */
    public LeshanServerBuilder2 setPublicKey(PublicKey publicKey) {
     ...
     */
    public LeshanServerBuilder2 setPrivateKey(PrivateKey privateKey) {
     ...
     * RPK support, use {@link LeshanServerBuilder2#setPublicKey(PublicKey)} instead.
     */
    public <T extends X509Certificate> LeshanServerBuilder2 setCertificateChain(T[] certificateChain) {
     ...
     */
    public <T extends Certificate> LeshanServerBuilder2 setTrustedCertificates(T[] trustedCertificates) {
     ...
     */
    public LeshanServerBuilder2 setEncoder(LwM2mEncoder encoder) {
     ...
     */
    public LeshanServerBuilder2 setDecoder(LwM2mDecoder decoder) {
     ...
     * 
     */
    public LeshanServerBuilder2 setCoapConfig(Configuration config) {
     ...
     */
    public LeshanServerBuilder2 setDtlsConfig(DtlsConnectorConfig.Builder config) {
     ...
     */
    public LeshanServerBuilder2 setEndpointFactory(EndpointFactory endpointFactory) {
     ...
     * with queue mode.
     */
    public LeshanServerBuilder2 disableQueueModeSupport() {
     ...
     */
    public LeshanServerBuilder2 setClientAwakeTimeProvider(ClientAwakeTimeProvider awakeTimeProvider) {
     ...
     */
    public LeshanServerBuilder2 setUpdateRegistrationOnNotification(boolean updateRegistrationOnNotification) {
     ...
     */
    public LeshanServer2 build() {
        if (localAddress == null)
            localAddress = new InetSocketAddress(LwM2m.DEFAULT_COAP_PORT);
        if (registrationStore == null)
            registrationStore = new InMemoryRegistrationStore2();
        if (authorizer == null)
        ...
        CaliforniumEndpointsProvider.Builder builder = new CaliforniumEndpointsProvider.Builder(
                new CoapProtocolProvider());
        builder.setCoapServerConfiguration(coapConfig);
        builder.addEndpoint(localAddress, new CoapEndpointFactory());
        if (shouldTryToCreateSecureEndpoint())
            builder.addEndpoint(localSecureAddress, new CoapsEndpointFactory(dtlsConfigBuilder));

        ServerSecurityInfo serverSecurityInfo = new ServerSecurityInfo(privateKey, publicKey, certificateChain,
                trustedCertificates);

        return createServer(builder.build(), registrationStore, securityStore, authorizer, modelProvider, encoder,
                decoder, coapConfig, noQueueMode, awakeTimeProvider, registrationIdProvider, linkParser,
                serverSecurityInfo);
    }
     ...
     * @param serverSecurityInfo
     * 
     * @return the LWM2M server
     */
    protected LeshanServer2 createServer(LwM2mEndpointsProvider endpointsProvider, RegistrationStore registrationStore,
            SecurityStore securityStore, Authorizer authorizer, LwM2mModelProvider modelProvider, LwM2mEncoder encoder,
            LwM2mDecoder decoder, Configuration coapConfig, boolean noQueueMode,
        ...
        return new LeshanServer2(endpointsProvider, registrationStore, securityStore, authorizer, modelProvider,
                encoder, decoder, noQueueMode, awakeTimeProvider, registrationIdProvider, noQueueMode, linkParser,
                serverSecurityInfo);
    }
}
