...
public class DefaultLwM2mRequestSender implements LwM2mRequestSender {
    ...
    public <T extends LwM2mResponse> T send(Registration destination, DownlinkRequest<T> request,
        ...
        LwM2mEndpoint endpoint = endpointsProvider.getEndpoint(destination.getLastEndpointUsed());

        // Retrieve the objects definition
        final LwM2mModel model = modelProvider.getObjectModel(destination);

        // Send requests synchronously
        T response = endpoint.send(new ClientProfile(destination, model), request, new LowerLayerSetting() {
            @Override
            public void apply(Object lowerLayerRequest) {
                if (lowerLayerConfig != null)
                    lowerLayerConfig.apply(lowerLayerRequest);
            }
        }, timeoutInMs);
        return response;
    }
    ...
    public <T extends LwM2mResponse> void send(final Registration destination, DownlinkRequest<T> request,
        ...
        }, errorCallback, new LowerLayerSetting() {
            @Override
            public void apply(Object lowerLayerRequest) {
                if (lowerLayerConfig != null)
                    lowerLayerConfig.apply(lowerLayerRequest);
    ...
    public void cancelOngoingRequests(Registration registration) {
        for (LwM2mEndpoint endpoint : endpointsProvider.getEndpoints()) {
