...
import org.eclipse.leshan.server.observation.ObservationServiceImpl2;
...
import org.eclipse.leshan.server.request.DefaultLwM2mRequestSender;
 ...
 * <p>
 * The {@link LeshanServerBuilder} should be the preferred way to build an instance of {@link LeshanServer2}.
 */
public class LeshanServer2 implements LwM2mServer {

    private static final Logger LOG = LoggerFactory.getLogger(LeshanServer2.class);
    ...
    private LwM2mEndpointsProvider endpointsProvider;

    /** @since 1.1 */
    protected /* final */ ObservationServiceImpl2 observationService;
     ...
     * <p>
     * {@link LeshanServerBuilder} is the priviledged way to create a {@link LeshanServer2}.
     *
     ...
     * @since 1.1
     */
    public LeshanServer2(LwM2mEndpointsProvider endpointsProvider, RegistrationStore registrationStore,
        ...
        // create endpoints
        LwM2mEndpointToolbox toolbox = new LwM2mEndpointToolbox(decoder, encoder, linkParser,
                new ServerPeerProfileProvider(registrationStore, modelProvider));
        RegistrationHandler registrationHandler = new RegistrationHandler(registrationService, authorizer,
                registrationIdProvider);
        ServerRequestReceiver requestReceiver = new ServerRequestReceiver(registrationHandler, sendService);
        endpointsProvider.createEndpoints(requestReceiver, observationService, toolbox, serverSecurityInfo, this);

        // create request sender
        requestSender = createRequestSender(endpointsProvider, registrationService, this.modelProvider, encoder,
                decoder, presenceService);
    ...
    protected ObservationServiceImpl2 createObservationService(RegistrationStore registrationStore,
            boolean updateRegistrationOnNotification, LwM2mEndpointsProvider endpointsProvider) {

        ObservationServiceImpl2 observationService = new ObservationServiceImpl2(registrationStore, endpointsProvider,
    ...
    protected LwM2mRequestSender createRequestSender(LwM2mEndpointsProvider endpointsProvider,
            RegistrationServiceImpl registrationService, LwM2mModelProvider modelProvider, LwM2mEncoder encoder,
            LwM2mDecoder decoder, PresenceServiceImpl presenceService) {

        // if no queue mode, create a "simple" sender
        final LwM2mRequestSender requestSender;
        if (presenceService == null)
            requestSender = new DefaultLwM2mRequestSender(endpointsProvider, modelProvider, encoder, decoder,
                    linkParser);
        else
            requestSender = new QueueModeLwM2mRequestSender(presenceService,
                    new DefaultLwM2mRequestSender(endpointsProvider, modelProvider, encoder, decoder, linkParser));
